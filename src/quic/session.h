...
class Session final : public AsyncWrap, private SessionTicket::AppData::Source {
 public:
  ...
  bool has_settings_listener() const;
 ...
 private:
  ...
  // an Application is first needed, so an application is always in place
  // before any stream or protocol event is processed. False if the
  // application could not be started, which is fatal to the session.
