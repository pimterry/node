...
// consumer (e.g. the HTTP/3 layer in internal/quic/http3).
...
// the HTTP/3 layer) to reach the binding handles of a stream or session:
...
function processSessionOptions(options, config = kEmptyObject) {
  ...
  // application's consumer layer, e.g. the HTTP/3 layer). Bad values are core
...
module.exports = {
  ...
  // consumer layer (e.g. the HTTP/3 layer). Never exported to users.
  kApplication,
