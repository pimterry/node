...
} = require('internal/tls/negotiation');
...
function processTlsOptions(tls, forServer) {
  const {
    servername,
    alpn,
    ciphers: cipherExpression = DEFAULT_CIPHERS,
  ...
  const ciphers = narrowCiphers(
    cipherExpression, 'options.ciphers', 'TLSv1.3', DEFAULT_CIPHERS);
  if (groups !== undefined) {
  ...
  if (certificateCompression !== undefined) {
    packedCertificateCompression = packCertificateCompression(
      certificateCompression, 'options.certificateCompression');
  }
