...
const {
  ...
  QUIC_APPLICATION_DEFAULT: kApplicationTypeDefault,
  QUIC_APPLICATION_PENDING: kApplicationTypePending,
...
// Internal-only symbol to expose the session #handle to applications
...
/**
 * Called when a SETTINGS frame is received from the remote peer.
 * @callback OnSettingsCallback
 * @this {QuicSession}
 * @param {Http3Settings} settings The settings now in effect
 * @returns {void}
 */
...
setCallbacks({
   ...
   * @param {Http3Settings} settings The settings now in effect
   */
  onSessionSettings(settings) {
    debug('session settings callback', this[kOwner]);
    this[kOwner][kSessionSettings](settings);
  },
...
class QuicSession {
  ...
  /**
   * Handle accessor to allow applications to reach the session handle.
   * Returns undefined once destroyed.
   * @type {object|undefined}
   */
  get [kSessionHandle]() {
  ...
  // HTTP/3-only, so no public equivalent: see internal/quic/http3.
  /** @type {Function|undefined} */
  get [kOnSettings]() {
    return this.#inner.onsettings;
  }

  set [kOnSettings](fn) {
    const inner = this.#inner;
    inner.onsettings =
      fn === undefined ? undefined : FunctionPrototypeBind(fn, this);
    inner.state.hasSettingsListener = fn !== undefined;
  }
  ...
  set [kOnOrigin](fn) {
    ...
    inner.onorigin =
      fn === undefined ? undefined : FunctionPrototypeBind(fn, this);
    inner.state.hasOriginListener = fn !== undefined;
  }
  ...
  // If an non-default application type is used, this session does not own the
  // streams - streams created here would not integrate into the application
  // and will generally stall or crash. Once an application is attached, the
  // application owns all stream creation.
  #assertOwnsStreams() {
    const type = this.#inner.state.applicationType & ~kApplicationTypePending;
    if (type > kApplicationTypeDefault) {
      throw new ERR_INVALID_STATE(
        'Raw QUIC streams cannot be created on a session with a non-default ' +
        'application attached. Create streams through the application ' +
        'interface (e.g. Http3Session) instead');
    }
  }

  async [kCreateStream](direction, options) {
    return await this.#createStream(direction, options);
  }
  ...
  /**
   * @param {Http3Settings} settings
   */
  [kSessionSettings](settings) {
    if (this.destroyed) return;
    if (onSessionSettingsChannel.hasSubscribers) {
      onSessionSettingsChannel.publish({
        __proto__: null,
        settings,
        session: this,
      });
    ...
    const inner = this.#inner;
    if (typeof inner.onsettings === 'function')
      safeCallbackInvoke(inner.onsettings, this, settings);
  }
...
function processTlsOptions(tls, forServer) {
  ...
  if (alpn !== undefined) {
    ...
    // Transport-only: there is no application protocol to default to.
...
module.exports = {
  ...
  DEFAULT_CIPHERS,
  DEFAULT_GROUPS,
  // Internal only, for the HTTP/3 layer's integration with node:quic.
  kCreateStream,
  kOnGoaway,
  kOnOrigin,
  kOnSettings,
