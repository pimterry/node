...
// Internal-only symbols shared with protocol applications and their consumer
// layers (e.g. HTTP/3, in internal/quic/http3). Not public options: an
// application installed without its consumer layer breaks the session.
const kApplication = Symbol('kApplication');
// Opaque here: parsed by the named application's own settings parser in C++.
...
// Brands a handle with its application-layer owner (e.g. an Http3Stream).
...
class QuicSession {
  ...
  static {
    ...
    // Blocks locally-initiated streams and datagrams without closing the
    // session, reflecting a peer-initiated graceful shutdown.
...
module.exports = {
  ...
  DEFAULT_CIPHERS,
  DEFAULT_GROUPS,
  // Internal-only hooks for protocol applications and their consumer layers.
  kApplication,
