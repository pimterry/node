...
class QuicSession {
   ...
   *
   * When an error is given without options, the close options are derived
   * from it so the peer still receives a CONNECTION_CLOSE.
   * @param {any} error
   * @param {object} [options]
   * @param {bigint|number} [options.code] The error code to send in the
   *   CONNECTION_CLOSE frame. Defaults to NO_ERROR (0).
  ...
  destroy(error, options) {
    ...
    if (options !== undefined) {
      options = validateCloseOptions(options);
    } else if (error !== undefined && inner.handshakeCompleted &&
               !inner.state.isClosing) {
      // Match QuicEndpoint.destroy() by deriving options from the error.
      options = errorToCloseOptions(error);
    }
    inner.destroying = true;
