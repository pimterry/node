...
const kHttp3Alpn = 'h3';
// NGHTTP3_H3_REQUEST_REJECTED (RFC 9114): sent on RESET_STREAM when an
// incoming request stream is dropped without any application processing.
...
class Http3Session {
   ...
   * @param {object} [options] settings plus creation-time session callbacks
   */
  constructor(session, options = kEmptyObject) {
    ...
    if (session.destroyed) {
      // Without this the session handle is already gone, and the wrapper
      // would be built with nothing behind it, failing silently later.
    ...
    session.onstream = (stream) => {
      if (typeof this.#onstream !== 'function') {
        process.emitWarning(
          ...
          'was provided');
        // Reset with H3_REQUEST_REJECTED so the peer learns the request
        // was not processed and may retry it (RFC 9114 section 4.1.1).
...
// C++ layer invokes each with `this` set to the handle the event belongs
// to: the stream handle for stream events, the session's HTTP/3 handle
// for session-level events; route it to the wrapper. Events for a handle
// with no live wrapper are dropped. This runs once per realm at module
// load: any HTTP/3 session requires this module, so the callbacks are
// always installed before an event can fire.
setHttp3Callbacks({
  /**
