...
const client = await connect('123.123.123.123:8888', { alpn: 'h3', endpoint });
```
...
If no `onstream` callback is set, an incoming stream is destroyed on
arrival and a warning is emitted.
...
Each connection is an {quic.Http3Session}, wrapping a [`QuicSession`][], and
each request/response exchange is an {quic.Http3Stream}, wrapping a
[`QuicStream`][].
...
* `options` {Object} HTTP/3 options. Other [`quic.connect()`][] session options
  ...
  * `settings` {quic.Http3Settings} Local HTTP/3 SETTINGS to advertise to the peer.
...
* Returns: {Promise} Fulfilled with an {quic.Http3Session}.
...
* `onsession` {quic.OnHttp3SessionCallback} Invoked with each new {quic.Http3Session}.
...
* Returns: {Promise} Fulfilled with an {quic.Http3Stream}.
...
* `maxHeaderPairs` {bigint|number} Maximum number of header pairs permitted
  for a stream. This is a local enforcement limit, not a wire setting.
* `maxHeaderLength` {bigint|number} Maximum total number of header bytes
  (name and value) permitted for a stream. This is a local enforcement
  limit, not a wire setting.
* `maxFieldSectionSize` {bigint|number} Maximum size, in bytes, of a header
  field section the endpoint will accept. **Default:** the value of
  `maxHeaderLength`; a lower value is raised to match it.
* `qpackMaxDTableCapacity` {bigint|number} Maximum QPACK dynamic table
  capacity. **Default:** `4096`.
* `qpackEncoderMaxDTableCapacity` {bigint|number} Maximum QPACK encoder
  dynamic table capacity. **Default:** `4096`.
* `qpackBlockedStreams` {bigint|number} Maximum number of streams that may be
  blocked waiting for QPACK state. **Default:** `100`.
...
[`http3session.settings`][]. Numeric values may be given as either a `bigint`
or a `number`, and are always read back as a `bigint`.
...
* Stream callbacks (`onblocked`, `onreset`, `onstopsending`): the stream is
  destroyed.
...
changes:
  - version: REPLACEME
    pr-url: https://github.com/nodejs/node/pull/65441
    description: The callback now receives the origins as an argument instead of being invoked with `this` bound to the session.
-->
...
changes:
  - version: REPLACEME
    pr-url: https://github.com/nodejs/node/pull/65441
    description: The callback now receives the headers as an argument instead of being invoked with `this` bound to the stream.
-->
...
* `settings` {quic.Http3Settings} The peer's HTTP/3 settings, merged with the
  local ones.
* `session` {quic.QuicSession}
